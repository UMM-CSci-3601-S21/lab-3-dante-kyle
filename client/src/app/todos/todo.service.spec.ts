...
describe('TodoService', () => {
  ...
  describe('getTodos()', () => {
    ...
    describe('Calling getTodos() with parameters correctly forms the HTTP request', () => {
      ...
      it('correctly calls api/todos with filter parameter \'owner\'', () => {
        todoService.getTodos({ owner: 'Tim' }).subscribe(
          todos => expect(todos).toBe(testTodos)
        );
        ...
        // Check that the role parameter was 'Tim'
        expect(req.request.params.get('owner')).toEqual('Tim');
  ...
  describe('filterTodos()', () => {
    ...
    it('filters by category', () => {
      ...
      // There should be 2 todos with homework as their category.
      expect(filteredTodos.length).toBe(2);
